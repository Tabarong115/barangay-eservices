...
def _add_enhanced_footer(pdf, request_data, margin):
    ...
    if request_data.get("secretary_signature_path"):
        ...
        if image_reader:
            pdf.drawImage(image_reader, sec_x + 2.5 * mm, sec_y - 17 * mm, width=50 * mm, height=18 * mm, preserveAspectRatio=True, mask="auto")
    ...
    pdf.setStrokeColor(NAVY)
    pdf.setLineWidth(0.8)
    pdf.line(sec_x + 2.5 * mm, sec_y - 24 * mm, sec_x + 52.5 * mm, sec_y - 24 * mm)
    
    # 5. Position
    pdf.setFont("Helvetica", 8.5)
    pdf.drawCentredString(sec_x + 27.5 * mm, sec_y - 28 * mm, "Barangay Secretary")
