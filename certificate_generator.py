...
def _add_enhanced_footer(pdf, request_data, margin):
    ...
    if qr_code:
        ...
        # Legal compliance footer text positioned above QR code to avoid overlap
        pdf.setFillColor(colors.HexColor("#6B7280"))
        pdf.setFont("Helvetica", 6.3)
        pdf.drawString(
            margin,
            30 * mm,
            "This QR-verifiable electronic certificate may be authenticated through the Barangay 7 e-Services Portal.",
        )
        pdf.drawString(
            margin,
            27 * mm,
            "Its electronic form is recognized under RA 8792 and A.M. No. 01-7-01-SC.",
        )
    else:
        # Fallback if QR code generation fails - center the footer text
        pdf.setFillColor(colors.HexColor("#6B7280"))
        pdf.setFont("Helvetica", 6.3)
...
def generate_business_closure_certification(request_data, output_path):
    ...
    # Extract reason (handle both reason and closure_reason field names)
