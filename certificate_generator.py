...
def _add_enhanced_footer(pdf, request_data, margin):
    ...
    pb_y = 85 * mm
    pb_x = PAGE_WIDTH - margin - 60 * mm
    
    # 1. "Approved by" text
    pdf.setFillColor(NAVY)
    pdf.setFont("Helvetica", 8)
    pdf.drawCentredString(pb_x + 30 * mm, pb_y, "Approved by")
    
    # 2. Signature image (if available)
    ...
    if punong_name:
        pdf.drawCentredString(pb_x + 30 * mm, pb_y - 22 * mm, punong_name)
    else:
        pdf.drawCentredString(pb_x + 30 * mm, pb_y - 22 * mm, "_____________________")
    
    # 4. Short Line
    pdf.setStrokeColor(NAVY)
    pdf.setLineWidth(1)
    pdf.line(pb_x + 5 * mm, pb_y - 26 * mm, pb_x + 55 * mm, pb_y - 26 * mm)
    ...
    pdf.setFillColor(NAVY)
    pdf.setFont("Helvetica", 8)
    pdf.drawCentredString(sec_x + 30 * mm, sec_y, "Attested by")
    ...
    if secretary_name:
        pdf.drawCentredString(sec_x + 30 * mm, sec_y - 22 * mm, secretary_name)
    else:
        pdf.drawCentredString(sec_x + 30 * mm, sec_y - 22 * mm, "_____________________")
    
    # 4. Short Line
    pdf.setStrokeColor(NAVY)
    pdf.setLineWidth(1)
    pdf.line(sec_x + 5 * mm, sec_y - 26 * mm, sec_x + 55 * mm, sec_y - 26 * mm)
    
    # 5. Position
    pdf.setFont("Helvetica", 9)
    pdf.drawCentredString(sec_x + 30 * mm, sec_y - 30 * mm, "Barangay Secretary")
